...
# 设置 OpenAI API 的 URL
openai.api_base = "https://api.gpt.ge/v1/chat/completions"  # 使用新版API的自定义基础URL
...
while True:
    ...
    if user_input.startswith("股票:"):  # 处理股票数据请求
        ...
        stock_summary = analyze_stock_data(ticker)
        print("分析结果:\n", stock_summary)
        continue
    ...
    try:
        completion = openai.chat.completions.create(  # 使用新版API的调用方式
            model="gpt-4o",  # 使用更高级的GPT模型
            messages=messages,
        )
        bot_response = completion['choices'][0]['text']  # 新版中使用 'text' 提取回复
